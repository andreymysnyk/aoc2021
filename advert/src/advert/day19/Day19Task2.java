...
public class Day19Task2 {
    ...
    private static class Match {
        ...
        public Point scanner;
        public Set<Point> uniquePoints;

        public static Match of(Function<Point, Point> rotation, Point scanner, Set<Point> uniquePoints) {
    ...
    private static Match getUniquePoints(List<Point> reportA, List<Point> reportB) {
        ...
        for (Function<Point, Point> rotation : rotations) {
            ...
            for (Line lineB : linesB) {
                ...
                if (pA.size() >= 12) {
                    pB.removeAll(pA);
                    Point scannerPoint = translate(translate(lineB.a, Point.of(0, 0, 0)), lineA.a);
                    return Match.of(rotation, scannerPoint, pB);
                }
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Point> scannerPoints = new ArrayList<>();
        while (visited.size() != reportList.size()) {
            for (int j = 0; j < reportList.size(); j++) {
                ...
                System.out.println(j + " " + reportJ.scanner);
                scannerPoints.add(reportJ.scanner);
        ...
        int max = 0;
        for (int i = 0; i < scannerPoints.size(); i++) {
            for (int j = i + 1; j < scannerPoints.size(); j++) {
                max = Math.max(max, distance(scannerPoints.get(i), scannerPoints.get(j)));
            }
        }
